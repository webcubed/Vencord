    ...
    <ErrorBoundary
      ...
      onError={() => {
        ...
        VencordNative.native.openExternal(
          "https://github.com/gujarathisampath/fakeProfile"
        );
      }}
    >
...
export default definePlugin({
  name: "fakeProfile",
  description:
    "Unlock Discord's and custom profile effects, profile themes, avatar decorations and badges without the need for Nitro",
  authors: [
  ...
  getProfileEffectById(
    skuId: string,
    effects: Record<string, ProfileEffectConfig>
  ) {
  ...
  profileDecodeHook(user: UserProfile) {
    if (user) {
      if (
        settings.store.enableProfileEffects ||
        settings.store.enableProfileThemes
      ) {
        console.log(user);
  ...
  useBannerHook({ displayProfile }: any) {
    ...
    if (
      UsersData[displayProfile?.userId] &&
      UsersData[displayProfile?.userId].banner
    )
      return UsersData[displayProfile?.userId].banner;
  },
  ...
  shouldShowBadge({ displayProfile, user }: any) {
    return (
      ...
      (UsersData[user.id] || settings.store.nitroFirst)
    );
  },
  getAvatarHook:
    (original: any) => (user: User, animated: boolean, size: number) => {
      ...
      if (animated) {
        ...
        if (avatarUrl && typeof avatarUrl === "string") {
          ...
          const image_name = parsedUrl.pathname
            .split("/")
            .pop()
            ?.replace("a_", "");
          return (
            BASE_URL + "/image/" + image_name ?? original(user, animated, size)
          );
        }
      }
      return original(user, animated, size);
  ...
  getAvatarDecorationURL({
    ...
    if (canAnimate && avatarDecoration?.animated !== false) {
      if (avatarDecoration?.skuId === SKU_ID) {
        const url = new URL(
          `${BASE_URL}/avatar-decoration-presets/a_${avatarDecoration?.asset}.png`
        );
        return url.toString();
      } else {
        const url = new URL(
      ...
      if (avatarDecoration?.skuId === SKU_ID) {
        const url = new URL(
          `${BASE_URL}/avatar-decoration-presets/${avatarDecoration?.asset}.png`
        );
        return url.toString();
      } else {
        const url = new URL(
          `https://cdn.discordapp.com/avatar-decoration-presets/${avatarDecoration?.asset}.png?passthrough=false`
        );
        return url.toString();
      }
    }
